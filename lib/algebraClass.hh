...
class AlgebraClass {
 public:
  ...
  float squareTwoNoise(float x) const {
    if (!noise_) throw std::runtime_error("No noise object found!");
  ...
  float longOperation(float x) const {
    ...
    if (has_logger) logger_->info("Long operationg starting...");

    float res = squareTwo(x);

    if (has_logger) logger_->info("Long operation over...");
